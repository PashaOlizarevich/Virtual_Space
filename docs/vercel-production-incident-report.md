...
устранены поэтапно. На момент составления отчёта сайт открывается, подключение к Neon и миграции
работают, обязательные настройки магазина созданы. Каталог остаётся пустым, поскольку
демонстрационные товары из репозитория ещё не загружены в production-базу.
...
Для полного соответствия содержимому репозитория требуется отдельная повторяемая data migration,
которая добавит отсутствующие категории, товары, изображения, характеристики и варианты, не удаляя
и не перезаписывая уже существующие операторские данные.
