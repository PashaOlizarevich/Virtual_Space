...
### 10. В превью-карточках отсутствовала галерея изображений

После загрузки production-каталога карточки отображали только основное изображение без стрелок и
счётчика слайдера. Данные галереи присутствовали в PostgreSQL, но публичный preview-запрос выбирал
только первое изображение, а DTO не передавал галерею клиентскому компоненту.

Выполнено в коде:

- preview-запрос возвращает всю упорядоченную галерею товара;
- публичный DTO валидирует и передаёт до 20 изображений;
- mapper сохраняет порядок изображений и использует первый элемент как основное изображение;
- добавлены regression-проверки серверного select, DTO и mapper;
- Prisma schema, миграции, production-данные и переменные Vercel не изменялись.

Исправление появится на production-сайте после следующего Production release.
