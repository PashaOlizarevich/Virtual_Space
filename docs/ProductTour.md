...
- Левое desktop-меню: «Каталог» (`/catalog`) и «О нас» (`/about`). Массив ссылок
  `navigationItems` находится в `header.tsx`; переход на главную доступен через логотип.
...
содержит пункт «Главная» (`/`), поэтому при изменении навигации нужно проверять оба массива.
