...
Экран состоит из интерьерного изображения и вкладок входа/регистрации. Вход выполняется через
Credentials Auth.js, регистрация — через Server Action с последующим входом. Ссылка «Забыли пароль?»
открывает модальное окно поверх экрана; запрос восстановления возвращает одинаковый публичный ответ
независимо от существования email. После входа гостевая корзина атомарно объединяется с
persistent-корзиной пользователя.
...
- Поля, переключение вкладок, модальное восстановление, сообщения и redirect:
  `src/modules/auth/components/auth-forms.tsx`.
