...
- Левое меню: «Главная» (`/`), «Каталог» (`/catalog`), «О нас» (`/about`). Массив ссылок
  `navigationItems` находится в `header.tsx`.
...
Важно: пункты основного меню продублированы в `header.tsx` и `mobile-navigation.tsx`. При добавлении
или переименовании пункта сейчас нужно изменить оба массива `navigationItems`.
