...
- [src/modules/cart/server-cart-adapter.ts](../src/modules/cart/server-cart-adapter.ts) преобразует
  server-owned DTO в локальную presentation-модель; актуальная цена заменяет недоверенное значение
  localStorage, а `slug` и основное изображение используются только для отображения.
...
- [src/modules/cart/server/merge-action.ts](../src/modules/cart/server/merge-action.ts) предоставляет
  ...
  `CART_CONFLICT` для интеграции входа.
...
конфликте серверная корзина остаётся неизменной. UI вызывает merge после Auth.js-входа, заменяет
Zustand server DTO и сериализует последующие update/remove операции.
...
- [Публичные auth Actions](../src/modules/auth/server/actions.ts) подключают регистрацию и запрос
  восстановления к `/login`; raw reset token не возвращается клиенту, а публичный ответ не раскрывает
  существование email.
