...
Контракт пока является внутренней доменной границей будущего order transport: он не создаёт заказ,
не изменяет остаток и не публикует отдельный endpoint. Клиентские цена, доступность и подписи опций
