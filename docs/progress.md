...
## Task 164 — API серверной корзины

- Результат: пункт 73 добавляет защищённые Server Actions получения persistent-корзины, изменения
  количества и удаления позиции; чтение одновременно ревалидирует активность товара, полный набор
  опций, Decimal-цену и суммарный остаток по конфигурациям.
- Безопасность: вход ограничен строгими Zod-схемами без клиентской цены и `optionsKey`; identity
  берётся из актуальной Auth.js session, каждый Prisma read/write ограничен владельцем, DTO не
  содержит Prisma-модели и внутренние id. Read-only `security-review` подтверждённых findings не
  выявил.
- Файлы: `src/modules/cart/server/{schemas,cart-service,actions}.ts`, новые unit-тесты и документация
  архитектуры, API-слоя, Product Tour и решений первого DB-релиза.
- Проверки: `npm run typecheck`, `npm run lint`, полный `npm test -- --runInBand` (86 suites,
  274 теста), новые тесты после финального изменения (2 suites, 7 тестов), `npm run build` и
  `git diff --check` прошли.
- Переменные окружения: новых нет; build использовал только одноразовые безопасные placeholder
  `DATABASE_URL`, `DATABASE_URL_UNPOOLED` и `AUTH_SECRET` без подключения к PostgreSQL.
- Архитектура: persistent-корзина остаётся server-only частью модуля `cart`; собственный UI получает
  тонкий Action transport, а гостевой Zustand и объединение при входе остаются пункту 74/76.
- Product Tour: раздел корзины дополнен фактическим server-only transport и статусом UI-интеграции.
- API overview: добавлены схемы, сервис, Actions, ownership и правила серверной ревалидации корзины.
...
## Task 187 — Защита публичного каталога от товаров без изображений

- Результат: публичный каталог пропускает активные записи с пустой галереей, а detail-запрос такого
  товара возвращает `null`. Одна некорректная production-запись больше не обрушает главную страницу,
  каталог и страницу товара на строгой проверке поля `image`.
- Причина: старый незавершённый административный сценарий оставил активный товар без `ProductImage`;
  mapper подставлял пустую строку, которую корректно отклонял публичный Zod DTO.
- Файлы: `src/modules/catalog/server/service.ts`,
  `src/modules/catalog/server/public-image-guard.test.ts`, `docs/progress.md`.
- Проверки: targeted Jest (3 suites, 9 тестов), ESLint и TypeScript прошли.
- База и окружение: schema, миграции, production-данные и переменные не изменялись. Защита совместима
  с существующими записями; проблемный товар остаётся доступен администратору для добавления фото или
  снятия с публикации.
- Security review: новых findings нет; строгий публичный DTO сохранён, внутренние данные и невалидные
...
## Task 191 — Исправление клиентского контракта обновления статуса заказа

- Результат: административный интерфейс принимает полный успешный ответ `PATCH` со значениями
  `orderNumber`, `status` и `updatedAt`, после чего обновляет локальное состояние заказа.
- Причина: строгая клиентская Zod-схема не содержала возвращаемое сервером поле `updatedAt` и отклоняла
  корректный HTTP 200 response до обновления React state.
- Файлы: `src/modules/admin/orders-transport.ts`,
  `src/modules/admin/orders-transport.test.ts`, `docs/progress.md`.
- Проверки: targeted Jest, lint и typecheck прошли. Targeted admin E2E локально не запущен: тестовая
  PostgreSQL на `127.0.0.1:5432` недоступна, поэтому подготовка схемы завершилась ошибкой Prisma schema engine.
- Безопасность: новых findings нет; строгая allowlist-валидация response DTO сохранена, добавлено только
  уже опубликованное сервером ISO datetime поле. Авторизация, входной статус и серверные переходы не менялись.
- База, окружение и архитектура: без изменений. Product Tour: без изменений. API overview: без изменений,
  поскольку серверный HTTP-контракт и маршрут не менялись; исправлен соответствующий ему клиентский parser.
